...
REM Pad kernel to 60KB
echo.
echo Padding kernel...
powershell -Command "$bytes = [IO.File]::ReadAllBytes('kernel32.bin'); $pad = New-Object byte[] 61440; [Array]::Copy($bytes, $pad, [Math]::Min($bytes.Length, 61440)); [IO.File]::WriteAllBytes('kernel32.bin', $pad)"
