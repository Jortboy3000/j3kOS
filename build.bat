...
REM Pad kernel to 40KB
echo.
echo Padding kernel...
powershell -Command "$bytes = [IO.File]::ReadAllBytes('kernel32.bin'); $pad = New-Object byte[] 40960; [Array]::Copy($bytes, $pad, [Math]::Min($bytes.Length, 40960)); [IO.File]::WriteAllBytes('kernel32.bin', $pad)"
